...
- 商品 API 載入失敗時，首頁會顯示原因提示與「重新載入」按鈕，不再永久停在載入中；重新載入期間會恢復載入提示。
- 搜尋與分類改為共用同一套篩選條件，兩者會同時生效；有套用條件時會顯示分類、搜尋文字、結果數與「清除條件」按鈕，零筆結果會顯示空狀態。
- 搜尋欄改用 `input` 事件，中文輸入、貼上及手機輸入後會立即更新結果。
- 登入、註冊與驗證碼區塊改為正式 `form`，支援 Enter 送出；彈窗支援 Esc 關閉。
- 登入、發送驗證碼及確認驗證送出期間，按鈕會顯示處理中並暫時停用，避免連點造成重複請求。
- 表單欄位補上可供螢幕閱讀器辨識的標籤及適當的 `autocomplete`；驗證碼欄位在手機上提示使用數字鍵盤。
- 驗證：`node --check script.js` 與 `node --check backend/server.js` 通過。Playwright 使用模擬 API 驗證載入失敗與重試、搜尋和分類交集、清除條件、零筆結果、Enter 送出、防重複送出及 Esc 關閉。
- 響應式檢查涵蓋 320、390、768、1440px；頁面、搜尋狀態列、錯誤提示及登入／註冊彈窗均未出現橫向溢位。
- 本次沒有修改後端、資料庫與測試階段的 `000000` 驗證碼，也沒有操作正式帳號、Render、SendGrid 或 MongoDB Atlas。
...
本節保留 2026-09-21 當時的檢查紀錄；完成狀態請以較新的「2026-09-23 首頁操作流程修復」為準，其餘項目不代表已授權一次實作所有改善。
...
- 本機先重現商品 API 無法連線後仍顯示載入中、搜尋覆蓋分類、Enter 不送出登入及 Esc 不關閉登入彈窗；其中商品載入、搜尋分類與表單彈窗問題已於 2026-09-23 修復。未登入收藏入口隱藏仍待處理。
- 未登入 Header 在 320、360、390、768、1440px 寬度未重現登入／註冊換行；目前 `style.css` 的 `#userArea:empty` 會隱藏空頭像容器。
- 上述瀏覽器檢查不等於實體手機、正式部署、實際寄信或完整後端測試。
- 目前建議先後順序：驗證流程恢復能力 → 個人資料載入保護 → 收藏一致性 → 未完成操作入口 → 視覺與操作細節。`000000` 移除安排在正式上線前處理。
...
- Render 冷啟動時，商品可能需要等待一段時間才會顯示；首頁目前已有載入中提示。
- 首頁商品載入失敗時已有錯誤訊息與重新載入按鈕。
...
- 登入、註冊與驗證目前已在送出期間顯示處理中並暫時停用；個人資料儲存仍待補上。
- 檢查帳號、密碼長度、Email 與電話格式。
- 登入、註冊與驗證已避免連點重複送出；其他表單仍需逐一檢查。
- 將成功、失敗提示統一成相同的畫面風格。
...
- 登入、註冊與驗證碼欄位已有對應的隱藏 `label`；其他表單仍需檢查。
- 登入、註冊與驗證碼表單已支援 Enter 送出。
- 首頁彈窗已支援 Esc 關閉。
- 搜尋或分類沒有結果時已顯示「沒有符合目前條件的商品」。
